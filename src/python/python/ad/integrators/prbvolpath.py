...
class PRBVolpathIntegrator(RBIntegrator):
        ...
        while loop(active):
            ...
            with dr.resume_grad(when=not is_primal):
                ...
                if self.use_nee:
                    ...
                    emitted, ds = self.sample_emitter(mei, si, scene, sampler,
                        medium, channel, active_e_medium, active_e_surface, mode=dr.ADMode.Primal)
                    ...
                    if not is_primal:
                        self.sample_emitter(mei, si, scene, nee_sampler, medium,
                            channel, active_e_medium, active_e_surface, adj_emitted=contrib, δL=δL, mode=mode)
    ...
    def sample_emitter(self, mei, si, scene, sampler, medium, channel,
                       active_medium, active_surface, adj_emitted=None,
                       δL=None, mode=None):
